...
/**
 * This component is basically a fork from React-osigma-v2's SearchControl
...
const SearchField: FC<{ filters: FiltersState }> = ({ filters }) => {
  ...
  const refreshValues = () => {
    ...
    if (!selected && search.length > 1) {
      osigma.getGraph().forEachNode((key: string, attributes: Attributes): void => {
  ...
  useEffect(() => {
    ...
    osigma.getGraph().setNodeAttribute(selected, "highlighted", true);
    const nodeDisplayData = osigma.getNodeDisplayData(selected);

    if (nodeDisplayData)
      osigma.getCamera().animate(
    ...
    return () => {
      osigma.getGraph().setNodeAttribute(selected, "highlighted", false);
    };
  }, [selected]);
