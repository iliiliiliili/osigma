 ...
 *    `#drawImage`
 * 2. Hide the osigma instance
 * 3. Create a new osigma instance similar to the initial one (dimensions,
 *    settings, graph, camera...), use it and kill it
 * This exemple uses this last solution.
 */
export default async function saveAsPNG(renderer: osigma, inputLayers?: string[]) {
  ...
  // dimensions actually are 2 * W and 2 * H. osigma properly deals with it, but
  ...
  // Instantiate osigma:
  const tmpRenderer = new osigma(renderer.getGraph(), tmpRoot, renderer.getSettings());
