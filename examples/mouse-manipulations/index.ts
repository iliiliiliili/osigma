...
// Retrieve the html document for osigma container
...
renderer.on("clickStage", ({ event }: { event: { x: number; y: number } }) => {
  // osigma (ie. graph) and screen (viewport) coordinates are not the same.
  // So we need to translate the screen x & y coordinates to the graph one by calling the osigma helper `viewportToGraph`
