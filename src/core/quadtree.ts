    ...
    while (true) {
        ...
        if (collisions === 0 && level === 0) {
            ...
            if (!hasWarnedTooMuchOutside && containers[OUTSIDE_BLOCK].length >= 5) {
                ...
                console.warn(
                    "osigma/quadtree.insertNode: At least 5 nodes are outside the global quadtree zone. " +
        ...
        if (collisions === 0)
            throw new Error(
                `osigma/quadtree.insertNode: no collision (level: ${level}, key: ${key}, x: ${x}, y: ${y}, size: ${size}).`,
            );

        // If we have 3 collisions, we have a geometry problem obviously
        if (collisions === 3)
            throw new Error(
                `osigma/quadtree.insertNode: 3 impossible collisions (level: ${level}, key: ${key}, x: ${x}, y: ${y}, size: ${size}).`,
            );
