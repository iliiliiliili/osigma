...
export default function iterate(
    ...
    const debugLog = debug
        ? (state: string) => {
              console.log({
                  ...
                  dx: NodeMatrix[2],
                  dy: NodeMatrix[3],
                  nodeMatrix: NodeMatrix.slice(0, 50),
                  edgeMatrix: EdgeMatrix.slice(0, 50),
              });
          }
        : (_: string) => false;
                ...
                if (RegionMatrix[r + REGION_FIRST_CHILD] >= 0) {
                    ...
                    if (RegionMatrix[r + REGION_NODE] < 0) {
                        ...
                        if (
                            ...
                            if (
                                NodeMatrix[
                                    RegionMatrix[r + REGION_NODE] + NODE_Y
                                ] < RegionMatrix[r + REGION_CENTER_Y]
                            ) {
                                // Top Left quarter
                                q = RegionMatrix[r + REGION_FIRST_CHILD];
                            ...
                            if (
                                ...
                                // Top Right quarter
                                q =
                                    RegionMatrix[r + REGION_FIRST_CHILD] +
                                    PPR * 2;
                            } else {
                                // Bottom Right quarter
                                q =
                        ...
                        RegionMatrix[r + REGION_MASS] =
                            NodeMatrix[
                                RegionMatrix[r + REGION_NODE] + NODE_MASS
                            ];
                        RegionMatrix[r + REGION_MASS_CENTER_X] =
                            NodeMatrix[RegionMatrix[r + REGION_NODE] + NODE_X];
                        RegionMatrix[r + REGION_MASS_CENTER_Y] =
        ...
        for (n = 0; n < order; n += PPN) {
            ...
            while (true) {
                if (RegionMatrix[r + REGION_FIRST_CHILD] >= 0) {
                    ...
                    if (rn >= 0 && rn !== n) {
                        xDist =
                            NodeMatrix[n + NODE_X] - NodeMatrix[rn + NODE_X];
                        yDist =
                            NodeMatrix[n + NODE_Y] - NodeMatrix[rn + NODE_Y];
    ...
    for (n = 0; n < order; n += PPN) {
        ...
        if (options.strongGravityMode) {
            ...
            if (distance > 0)
                factor =
                    (coefficient * NodeMatrix[n + NODE_MASS] * g) / distance;
        }
    ...
    coefficient =
        1 *
        (options.outboundAttractionDistribution ? outboundAttCompensation : 1);
        ...
        if (adjustSizes === true) {
            ...
            if (options.linLogMode) {
                ...
                if (options.outboundAttractionDistribution) {
                    ...
                    if (distance > 0) {
                        factor =
                            (-coefficient * ewc) / NodeMatrix[n1 + NODE_MASS];
                    }
                } else {
    ...
    // 5) Apply Forces
    //-----------------
    let force, swinging, traction, nodespeed, newX, newY;
        ...
        for (n = 0; n < order; n += PPN) {
            if (NodeMatrix[n + NODE_FIXED] !== 1) {
                ...
                swinging =
                    NodeMatrix[n + NODE_MASS] *
                    Math.sqrt(
                        (NodeMatrix[n + NODE_OLD_DX] -
                            NodeMatrix[n + NODE_DX]) *
                            (NodeMatrix[n + NODE_OLD_DX] -
                                NodeMatrix[n + NODE_DX]) +
                            (NodeMatrix[n + NODE_OLD_DY] -
                                NodeMatrix[n + NODE_DY]) *
                                (NodeMatrix[n + NODE_OLD_DY] -
                                    NodeMatrix[n + NODE_DY])
                    );
